...
test_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-command-directory.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o \
		"$temporary_dir"/command-directory-m32 \
		"$temporary_dir"/command-directory-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/libc32/string.c" \
		"$c32_dir/command/directory.c" \
		"$test_dir/command_directory_test.c"
	do
	...
	"$test_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		"$temporary_dir/directory-$model.o" \
		"$temporary_dir/command_directory_test-$model.o" \
		-o "$temporary_dir/command-directory-m$model"
done

"$temporary_dir/command-directory-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/command-directory-m32"
fi

echo "protected COMMAND CHDIR tests passed: drive query/switch versus root change (m64 run, m32 link)"

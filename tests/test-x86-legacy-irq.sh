...
irq_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-native-irq.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/irq-m32 \
		"$temporary_dir"/irq-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in \
		...
		"$root_dir/kernel/x86_legacy_irq.c" \
		"$test_dir/x86_legacy_irq_test.c"
	do
	...
	"$irq_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		"$temporary_dir/x86_legacy_irq-$model.o" \
		"$temporary_dir/x86_legacy_irq_test-$model.o" \
		-o "$temporary_dir/irq-m$model"
done

"$temporary_dir/irq-m64"
m32_result="linked only"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/irq-m32"
	m32_result="executed"
fi

echo "x86 native-IRQ owner tests passed: m64 executed, m32 $m32_result"
echo "Covered sparse discovery, action-before-unmask, spurious IRQ and exact EOI"

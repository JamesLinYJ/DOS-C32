...
mutation_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-environment-mutation.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/environment-mutation-*
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in \
		...
		"$c32_dir/dos/environment/mutation.c" \
		"$test_dir/dos_environment_mutation_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}").m$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument list.
		"$mutation_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
		objects="$objects $object"
	done
	# shellcheck disable=SC2086 # Deliberate object list.
	"$mutation_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none $objects \
		-o "$temporary_dir/environment-mutation-$model"
done

"$temporary_dir/environment-mutation-64"
if command -v qemu-i386 >/dev/null 2>&1
then
	qemu-i386 "$temporary_dir/environment-mutation-32"
fi

echo "DOS environment mutation tests passed: set/delete, allocation, publication, rollback and poison"

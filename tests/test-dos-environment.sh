...
environment_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-environment.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-environment-m32 \
		"$temporary_dir"/dos-environment-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" \
		...
		"$test_dir/dos_environment_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$environment_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$environment_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/machine-$model.o" \
		"$temporary_dir/exec_name-$model.o" \
		"$temporary_dir/environment-$model.o" \
		"$temporary_dir/dos_environment_test-$model.o" \
		-o "$temporary_dir/dos-environment-m$model"
done

"$temporary_dir/dos-environment-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-environment-m32"
fi

echo "DOS environment tests passed: guest name, guest-byte order, layout, bounds, poison"

...
error_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-error.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-error-m32 \
		"$temporary_dir"/dos-error-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/error.c" "$test_dir/dos_error_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$error_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$error_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/error-$model.o" \
		"$temporary_dir/dos_error_test-$model.o" \
		-o "$temporary_dir/dos-error-m$model"
done

"$temporary_dir/dos-error-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-error-m32"
fi

echo "DOS error tests passed: legacy AX mapping and extended metadata"

...
shell_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-shell-capacity.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/shell-capacity-m32 \
		"$temporary_dir"/shell-capacity-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/libc32/string.c" "$c32_dir/dos/ui.c" \
		"$c32_dir/dos/drive/config.c" \
		"$c32_dir/dos/path.c" \
		"$c32_dir/shell/external_command.c" \
		"$c32_dir/shell/shell.c" \
		"$test_dir/shell_capacity_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$shell_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$shell_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/string-$model.o" \
		"$temporary_dir/ui-$model.o" \
		"$temporary_dir/config-$model.o" \
		"$temporary_dir/path-$model.o" \
		"$temporary_dir/external_command-$model.o" \
		"$temporary_dir/shell-$model.o" \
		"$temporary_dir/shell_capacity_test-$model.o" \
		-o "$temporary_dir/shell-capacity-m$model"
done

"$temporary_dir/shell-capacity-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/shell-capacity-m32"
fi

echo "shell capacity tests passed: bounded public lines and counted IOMGR paths (m64 run, m32 link)"

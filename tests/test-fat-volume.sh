...
fat_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-fat-volume.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/fat-volume-m32 \
		"$temporary_dir"/fat-volume-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/storage/fat/entry.c" \
		"$c32_dir/storage/fat/boot.c" \
		"$test_dir/fat_volume_test.c"
	do
	...
	"$fat_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		"$temporary_dir/fat_volume_test-$model.o" \
		-o "$temporary_dir/fat-volume-m$model"
done

"$temporary_dir/fat-volume-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/fat-volume-m32"
fi

echo "FAT volume tests passed: FAT12/16/32 BPB geometry and transactional outputs"

...
vector_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-vectors.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-vectors-m32 \
		"$temporary_dir"/dos-vectors-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" "$c32_dir/dos/vectors.c" \
		"$test_dir/dos_vectors_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$vector_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$vector_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/machine-$model.o" \
		"$temporary_dir/vectors-$model.o" \
		"$temporary_dir/dos_vectors_test-$model.o" \
		-o "$temporary_dir/dos-vectors-m$model"
done

"$temporary_dir/dos-vectors-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-vectors-m32"
fi

echo "DOS vector tests passed: byte layout, full range, bounds, rollback"

...
parameter_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-exec-parameter.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-exec-parameter-test-*
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" \
		...
		"$test_dir/dos_exec_parameter_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}").m$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument list.
		"$parameter_cc" -m$model $warning_flags -I"$c32_dir/include" \
			-c "$source" -o "$object"
		objects="$objects $object"
	done
	# shellcheck disable=SC2086 # Deliberate object list.
	"$parameter_cc" -m$model -nostdlib -no-pie -Wl,-e,_start \
		-Wl,--build-id=none $objects \
		-o "$temporary_dir/dos-exec-parameter-test-$model"
done

"$temporary_dir/dos-exec-parameter-test-64"

echo "dos-exec-parameter tests passed: ordered environment selection and late EXEC3 fields, 16-bit/A20 wrap (m64 run, m32 link)"

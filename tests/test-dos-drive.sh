...
drive_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-drive.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-drive-m32 \
		"$temporary_dir"/dos-drive-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/libc32/string.c" \
		"$c32_dir/dos/drive/config.c" "$test_dir/dos_drive_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$drive_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$drive_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/string-$model.o" \
		"$temporary_dir/config-$model.o" \
		"$temporary_dir/dos_drive_test-$model.o" \
		-o "$temporary_dir/dos-drive-m$model"
done

"$temporary_dir/dos-drive-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-drive-m32"
fi

echo "DOS drive tests passed: runtime namespace, designators and bounded paths"

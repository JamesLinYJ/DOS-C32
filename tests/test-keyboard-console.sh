...
keyboard_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-keyboard-console.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/keyboard-console-m32 \
		"$temporary_dir"/keyboard-console-m64
	rmdir -- "$temporary_dir"
}
...
common_flags="$common_flags -DDOSC32_HOST_TEST=1"
...
do
	...
	for source in \
		...
		"$c32_dir/kernel/input/console/keymap_us.c" \
		"$c32_dir/kernel/input/console/wait_x86.c" \
		"$c32_dir/kernel/keyboard.c" \
		"$test_dir/keyboard_console_test.c"
	do
		object="$temporary_dir/object-$object_index-$model.o"
		object_index=$((object_index + 1))
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$keyboard_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
		objects="$objects $object"
	done
	# shellcheck disable=SC2086 # Deliberate object/argument lists.
	"$keyboard_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none $objects \
		-o "$temporary_dir/keyboard-console-m$model"
done

"$temporary_dir/keyboard-console-m64"
m32_result="linked only"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/keyboard-console-m32"
	m32_result="executed"
fi

echo "keyboard-console tests passed: m64 executed, m32 $m32_result"
echo "Covered event-time BIOS AX, modifiers, locks, extended keys, wait, backpressure and teardown"

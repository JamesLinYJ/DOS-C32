...
dpb_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-dpb.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-dpb-m32 \
		"$temporary_dir"/dos-dpb-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/drive/dpb.c" "$test_dir/dos_dpb_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$dpb_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$dpb_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/dpb-$model.o" \
		"$temporary_dir/dos_dpb_test-$model.o" \
		-o "$temporary_dir/dos-dpb-m$model"
done

"$temporary_dir/dos-dpb-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-dpb-m32"
fi

echo "DOS DPB tests passed: layout, driver pointer, bounds and i386 data model"

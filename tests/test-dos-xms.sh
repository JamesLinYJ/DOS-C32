...
test_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-xms.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-xms-*
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" \
		...
		"$c32_dir/dos/xms/manager.c" "$test_dir/dos_xms_test.c"
	do
	...
	"$test_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		-o "$temporary_dir/dos-xms-$model"
done

"$temporary_dir/dos-xms-64"
if command -v qemu-i386 >/dev/null 2>&1
then
	qemu-i386 "$temporary_dir/dos-xms-32"
fi

echo "DOS XMS tests passed: HMA lease, allocation, handles and verified A20"

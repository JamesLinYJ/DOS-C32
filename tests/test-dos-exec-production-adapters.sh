...
test_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-exec-adapters.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/exec-adapters-*
	rmdir -- "$temporary_dir"
}
...
do
	...
	objects=""
	for source in "$c32_dir/dos/exec_gate.c" \
		"$c32_dir/dos/drive_visibility.c" \
		"$c32_dir/dos/sft_adapter.c" \
		"$c32_dir/dos/sft_batch.c" \
		"$test_dir/dos_exec_production_adapters_test.c"
	do
	...
	"$test_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		-o "$temporary_dir/exec-adapters-$model"
done

"$temporary_dir/exec-adapters-64"
if command -v qemu-i386 >/dev/null 2>&1
then
	qemu-i386 "$temporary_dir/exec-adapters-32"
fi

echo "production EXEC adapter tests passed: gate, SFT references and drive visibility"

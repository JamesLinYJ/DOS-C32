...
relocator_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-relocator.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/dos-relocator-m32 \
		"$temporary_dir"/dos-relocator-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" "$c32_dir/dos/relocator.c" \
		"$test_dir/dos_relocator_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$relocator_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$relocator_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/machine-$model.o" \
		"$temporary_dir/relocator-$model.o" \
		"$temporary_dir/dos_relocator_test-$model.o" \
		-o "$temporary_dir/dos-relocator-m$model"
done

"$temporary_dir/dos-relocator-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-relocator-m32"
fi

echo "DOS relocator tests passed: exact relocation-entry/empty-table seeks, two-pass bounds, rollback, poison (m64 run, m32 link)"

...
fat_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-fat-table.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/fat-table-m32 \
		"$temporary_dir"/fat-table-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	"$fat_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		"$temporary_dir/fat_table_test-$model.o" \
		-o "$temporary_dir/fat-table-m$model"
done

"$temporary_dir/fat-table-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/fat-table-m32"
fi

echo "FAT table tests passed: packing, classification, mirror LBAs and unchanged failure outputs"

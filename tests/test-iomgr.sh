...
iomgr_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-iomgr.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/iomgr-m32 \
		"$temporary_dir"/iomgr-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/storage/core/manager.c" \
		"$test_dir/iomgr_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$iomgr_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$iomgr_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/manager-$model.o" \
		"$temporary_dir/iomgr_test-$model.o" \
		-o "$temporary_dir/iomgr-m$model"
done

"$temporary_dir/iomgr-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/iomgr-m32"
fi

echo "I/O Manager tests passed: driver selection, volume generations and quarantine"

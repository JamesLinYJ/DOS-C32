...
journal_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-exec-journal.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o \
		"$temporary_dir"/dos-exec-journal-m32 \
		"$temporary_dir"/dos-exec-journal-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/dos/machine.c" \
		...
		"$test_dir/dos_exec_journal_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$journal_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$journal_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/machine-$model.o" \
		"$temporary_dir/exec_journal-$model.o" \
		"$temporary_dir/dos_exec_journal_test-$model.o" \
		-o "$temporary_dir/dos-exec-journal-m$model"
done

"$temporary_dir/dos-exec-journal-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/dos-exec-journal-m32"
fi

echo "DOS EXEC journal tests passed: far/A20 undo, capacity, LIFO, poison, pure seal (m64 run, m32 link)"

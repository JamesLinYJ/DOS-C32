...
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-iomgr-transaction.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/transaction-m32 \
		"$temporary_dir"/transaction-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/storage/core/manager.c" \
		...
		"$test_dir/iomgr_transaction_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$transaction_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$transaction_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/manager-$model.o" \
		"$temporary_dir/transaction-$model.o" \
		"$temporary_dir/iomgr_transaction_test-$model.o" \
		-o "$temporary_dir/transaction-m$model"
done

"$temporary_dir/transaction-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/transaction-m32"
fi

echo "I/O Manager transaction tests passed: commit, rollback and quarantine"

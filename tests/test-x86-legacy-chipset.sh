...
chipset_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-chipset.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/chipset-m32 \
		"$temporary_dir"/chipset-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in \
		...
		"$test_dir/x86_legacy_chipset_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$chipset_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$chipset_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/resource-$model.o" \
		"$temporary_dir/math64-$model.o" \
		"$temporary_dir/policy-$model.o" \
		"$temporary_dir/owner-$model.o" \
		"$temporary_dir/dma-$model.o" \
		"$temporary_dir/pic-$model.o" \
		"$temporary_dir/pit-$model.o" \
		"$temporary_dir/rtc-$model.o" \
		"$temporary_dir/x86_legacy_chipset_test-$model.o" \
		-o "$temporary_dir/chipset-m$model"
done

"$temporary_dir/chipset-m64"
m32_result="linked only"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/chipset-m32"
	m32_result="executed"
fi

echo "x86 legacy-chipset tests passed: m64 executed, m32 $m32_result"
echo "Covered isolated 8237 DMA registers, PIC delivery, PIT and RTC"

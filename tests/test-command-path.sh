...
test_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-command-path.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/command-path-m32 \
		"$temporary_dir"/command-path-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in "$c32_dir/libc32/string.c" "$c32_dir/command/path.c" \
		"$test_dir/command_path_test.c"
	do
	...
	"$test_cc" $architecture_flags -nostdlib -static -no-pie \
		...
		"$temporary_dir/command_path_test-$model.o" \
		-o "$temporary_dir/command-path-m$model"
done

"$temporary_dir/command-path-m64"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/command-path-m32"
fi

echo "protected COMMAND PATH parser tests passed: display/update/clear and transactional errors (m64 run, m32 link)"

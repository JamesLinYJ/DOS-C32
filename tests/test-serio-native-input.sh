...
serio_cc=${CC:-gcc}
temporary_dir=$(mktemp -d "${TMPDIR:-/tmp}/dos-c32-serio.XXXXXX")

cleanup()
{
	rm -f -- "$temporary_dir"/*.o "$temporary_dir"/serio-m32 \
		"$temporary_dir"/serio-m64
	rmdir -- "$temporary_dir"
}
...
do
	...
	for source in \
		...
		"$c32_dir/kernel/x86_vm/platform/native_input.c" \
		"$test_dir/serio_native_input_test.c"
	do
		object="$temporary_dir/$(basename "${source%.c}")-$model.o"
		# shellcheck disable=SC2086 # Deliberate compiler argument lists.
		"$serio_cc" $architecture_flags $common_flags \
			-I"$c32_dir/include" -I"$test_dir" \
			-c "$source" -o "$object"
	done
	# shellcheck disable=SC2086 # Deliberate compiler argument lists.
	"$serio_cc" $architecture_flags -nostdlib -static -no-pie \
		-Wl,-e,_start -Wl,--build-id=none \
		"$temporary_dir/registry-$model.o" \
		"$temporary_dir/binding-$model.o" \
		"$temporary_dir/dispatch-$model.o" \
		"$temporary_dir/native_input-$model.o" \
		"$temporary_dir/serio_native_input_test-$model.o" \
		-o "$temporary_dir/serio-m$model"
done

"$temporary_dir/serio-m64"
m32_result="linked only"
if command -v qemu-i386 >/dev/null 2>&1; then
	qemu-i386 "$temporary_dir/serio-m32"
	m32_result="executed"
fi

echo "serio/native-input tests passed: m64 executed, m32 $m32_result"
echo "Covered lifecycle rollback, direct IRQ/EOI results and bounded tri-state writes"
echo "Covered zero-commit backpressure and exact stream-loss recovery/isolation"
echo "Covered FIFO generations and subtree teardown"
